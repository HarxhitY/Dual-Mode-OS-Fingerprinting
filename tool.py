...
def process_packet(packet):
    with warnings.catch_warnings():
        ...
        if packet_info and len(packet_info) == 8:
            ...
            import numpy as np
            # Filter rows where os_name matches the prediction
            filtered_data = df[df.iloc[:, 0] == predicted_labels[0]]

            # Define the features column indices to match with the data array given
            features_to_match = [11, 5, 2, 12, 26, 25, 21, 19]

            # Initialize variables to track best matching row and minimum difference
            best_matching_row = None
            min_difference = np.inf

            # Iterate over rows to find the best match
            ...
            # Print the os_version of the best matching row
            print('Predicted Minor Version : ',best_matching_row[1])
            print('='*50)
            

        else:
            
            subprocess.run(["python3", "localAnalysis.py"], check=True)
